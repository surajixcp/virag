...
const Model = require('../models/lifeStyle.model');
...
module.exports = {
    ...
    getInfo: async (req, res) => res
        .status(200)
        .json({ message: `${ADMIN_SERVICE_WELCOME_MSG(ModuleName)} Info Route Working` }),
    create: async (req, res, next) => {
        try {
            ...
            if (doesExist) {
                throw createError.Conflict(`Data already exist ${JSON.stringify(doesExist)}`);
            }
            const model = new Model(data);
            const savedModel = await model.save();
            await UserModel.findByIdAndUpdate({ _id: mongoose.Types.ObjectId(data.user_id) }, { $set: { is_oldUser: true } });
            // TODO: Set notifications for super admin approve this service
            if (savedModel) {
                return res
                    .status(200)
                    .json({ success: true, status: 200, message: 'Data Inserted Successfully' });
            }
    ...
    updateById: async (req, res, next) => {
        try {
            const Id = req.params;
            const data = req.body;
            const fondedData = await Model.findOne({
                _id: mongoose
                    .Types
                    .ObjectId(Id)
            });
            ...
            if (result) {
                return res
                    .status(200)
                    .json({ success: true, status: 200, message: 'Data Updated Successfully' });
            }
    ...
    getList: async (req, res, next) => {
        try {
            ...
            list = await Model.aggregate([
                ...
                // {   $addFields: {     convertedUserId: { $toObjectId: "$user_id" }   } },
    ...
    getDataById: async (req, res, next) => {
        try {
            ...
            if (result) {
                return res
                    .status(200)
                    .json({ success: true, status: 200, message: 'Detail Fetched', data: result[0] });
            }
            return next(createError.BadRequest('Failed to update data.'));
        } catch (error) {
            return next(error);
    ...
    restoreDataById: async (req, res, next) => {
        try {
            ...
            if (result) {
                return res
                    .status(200)
                    .json({ success: true, message: 'Data restored successfully' });
            }
