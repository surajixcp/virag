...
const Model = require('../models/profile.visitor.model');
const { ADMIN_SERVICE_WELCOME_MSG } = require('../helpers/resource/constants');

const ModuleName = 'Profile Visitor';
module.exports = {
    /**
   ...
   * @param {Object} res - Express response object.
    ...
    profileVisit: async (req, res, next) => {
        try {
            const data = req.body;
            data.created_at = new Date();
            data.updated_at = new Date();
    ...
    getVisitorsByProfileId: async (req, res, next) => {
        try {
            let id = {};
            if (req.user && req.user.id) {
                id = req.user.id;
